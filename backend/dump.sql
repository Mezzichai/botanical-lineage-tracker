...
CREATE TABLE species (
  ...
  light_schedule jsonb DEFAULT '[{"hours": 12, "month": "January"}, {"hours": 12, "month": "February"}, 
  ...
  water_schedule jsonb DEFAULT '[{"water_count": 1, "month": "January"}, {"water_count": 1, "month": "February"}, 
...
CREATE TABLE parent_pairs (
  id serial PRIMARY KEY,
  mother_id integer REFERENCES individual_plant(id) ON DELETE CASCADE,
...
CREATE TABLE child_parent_pairs (
  individual_plant_id integer REFERENCES individual_plant(id) ON DELETE CASCADE,
  parent_pair_id integer REFERENCES parent_pairs(id) ON DELETE CASCADE,
  PRIMARY KEY (individual_plant_id, parent_pair_id)
);
